...
const S: Record<string, CSSProperties> = {
  ...
  th: { fontFamily: 'Space Grotesk', fontSize: '10px', fontWeight: 600, color: '#555', textTransform: 'uppercase', letterSpacing: '0.08em', padding: '10px 14px', textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.08)', whiteSpace: 'nowrap' },
  td: { fontFamily: 'Space Grotesk', fontSize: '12px', color: '#e5e2e1', padding: '10px 14px', borderBottom: '1px solid rgba(255,255,255,0.04)', whiteSpace: 'nowrap' },
}

function filterBtnStyle(active: boolean): CSSProperties {
  return {
    padding: '5px 12px',
    backgroundColor: active ? '#C08532' : 'transparent',
    color: active ? '#000' : '#737373',
    border: `1px solid ${active ? '#C08532' : 'rgba(255,255,255,0.1)'}`,
    ...
    <div style={S.page}>
      <div style={S.header}>
        <div>
          ...
          <p style={S.sub}>Authorization history · {MOCK.length} total events</p>
        </div>
        <div style={S.toolbar}>
          <input
            style={S.searchInput}
          ...
          <tbody>
            ...
            {filtered.map((r, i) => (
              <tr key={r.id} style={{ backgroundColor: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                ...
                <td style={S.td}>
                  <span style={{
                    padding: '2px 8px', borderRadius: '2px', fontSize: '10px', fontWeight: 700, letterSpacing: '0.06em',
                    backgroundColor: r.decision === 'APPROVED' ? 'rgba(74,225,118,0.1)' : 'rgba(255,180,171,0.1)',
                    color: r.decision === 'APPROVED' ? '#4ae176' : '#ffb4ab',
                    border: `1px solid ${r.decision === 'APPROVED' ? 'rgba(74,225,118,0.2)' : 'rgba(255,180,171,0.2)'}`,
                  }}>
                    {r.decision}
                  </span>
                </td>
