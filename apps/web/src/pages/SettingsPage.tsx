...
const S: Record<string, React.CSSProperties> = {
  ...
  cardTitle: { fontFamily: 'Space Grotesk', fontSize: '11px', fontWeight: 600, color: '#fff', textTransform: 'uppercase' as const, letterSpacing: '0.08em', borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: '10px' },
  ...
  toggle: (on: boolean): React.CSSProperties => ({ width: '36px', height: '20px', backgroundColor: on ? '#C08532' : '#1A1A1A', border: `1px solid ${on ? '#C08532' : 'rgba(255,255,255,0.1)'}`, borderRadius: '10px', cursor: 'pointer', position: 'relative', transition: 'all 0.2s', display: 'inline-block' }),
  toggleKnob: (on: boolean): React.CSSProperties => ({ position: 'absolute', top: '2px', left: on ? '18px' : '2px', width: '14px', height: '14px', backgroundColor: '#fff', borderRadius: '50%', transition: 'left 0.2s' }),
  ...
  btn: { padding: '8px 16px', backgroundColor: '#C08532', color: '#000', border: 'none', fontFamily: 'Space Grotesk', fontSize: '10px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' as const, cursor: 'pointer', borderRadius: '2px' },
  btnDanger: { padding: '8px 16px', backgroundColor: 'transparent', color: '#ffb4ab', border: '1px solid rgba(255,180,171,0.3)', fontFamily: 'Space Grotesk', fontSize: '10px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' as const, cursor: 'pointer', borderRadius: '2px' },
};
    ...
    <div style={S.page}>
      ...
      <div style={S.card}>
        ...
        <div>
          ...
          <input style={S.input} defaultValue={import.meta.env.VITE_API_URL || 'http://localhost:8000'}
      ...
      <div style={S.card}>
        ...
        {[
          { label: 'Auth denied alerts', desc: 'Notify when a request is denied', default: true },
          { label: 'Daily spend summary', desc: 'Email summary of daily agent spend', default: false },
          { label: 'Key rotation reminders', desc: 'Alert 7 days before key expiry', default: true },
        ].map(item => (
          <div key={item.label} style={S.row}>
            ...
            <div style={S.toggle(item.default)}>
              <div style={S.toggleKnob(item.default)} />
            </div>
          </div>
        ))}
      </div>
